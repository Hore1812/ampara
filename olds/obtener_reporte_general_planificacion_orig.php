...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $datos_participantes = [];
        if (!empty($idcliente_filtro)) {
            // Obtener los idContratoCliente para el idcliente dado
            $stmt_contratos_participantes = $pdo->prepare("SELECT idcontratocli FROM contratocliente WHERE idcliente = :idcliente");
            $stmt_contratos_participantes->execute([':idcliente' => $idcliente_filtro]);
            $contratos_ids_participantes = $stmt_contratos_participantes->fetchAll(PDO::FETCH_COLUMN);

            if ($contratos_ids_participantes) {
                $in_placeholders = implode(',', array_fill(0, count($contratos_ids_participantes), '?'));
                
                $sql_participantes = "SELECT NombreParticipante, SUM(HorasCompletadasPorParticipante) as HorasCompletadas 
                                      FROM vista_planificacion_vs_participantes_completado 
                                      WHERE idContratoCliente IN ($in_placeholders)";
                
                $params_participantes = $contratos_ids_participantes;

                if (!empty($anio)) {
                    $sql_participantes .= " AND SUBSTRING(MesPlan, 1, 4) = ?";
                    $params_participantes[] = $anio;
                }
                if (!empty($mes_input)) {
                    $mes_formateado = str_pad($mes_input, 2, '0', STR_PAD_LEFT);
                    $sql_participantes .= " AND SUBSTRING(MesPlan, 6, 2) = ?";
                    $params_participantes[] = $mes_formateado;
                }

                $sql_participantes .= " GROUP BY NombreParticipante ORDER BY HorasCompletadas DESC";

                $stmt_participantes = $pdo->prepare($sql_participantes);
                $stmt_participantes->execute($params_participantes);
                $datos_participantes = $stmt_participantes->fetchAll(PDO::FETCH_ASSOC);
            }
        }
        ...
        $response['data'] = $default_data_structure; // Enviar estructura vacía en error de BD
