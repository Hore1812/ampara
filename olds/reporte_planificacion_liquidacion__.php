...
$(document).ready(function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.success && response.data) {
                    ...
                    renderizarGraficoBarras(response.data.contratos);
    ...
    function renderizarTablaDobleEntrada(data) {
        ...
        data.forEach(item => {
            let bodyRow = `<tr><td>${item.contrato_cliente}</td><td>${parseFloat(item.horas_planificadas).toFixed(2)}h</td>`;
            estadosArray.forEach(estado => {
                const horas = parseFloat(item.estados[estado] || 0);
                const porcentaje = (horas / parseFloat(item.horas_planificadas) * 100);
            ...
            totalHorasPlanificadas += parseFloat(item.horas_planificadas);
        });
    ...
    function renderizarGraficoBarras(data) {
        ...
        const labels = data.map(item => item.contrato_cliente);
        const estados = new Set();
        data.forEach(item => {
            for (const estado in item.estados) {
                estados.add(estado);
        ...
        estadosArray.forEach((estado, index) => {
            ...
            datasets.push({
                label: estado,
                data: data.map(item => parseFloat(item.estados[estado] || 0)),
        ...
        datasets.unshift({
            ...
            data: data.map(item => parseFloat(item.horas_planificadas)),
